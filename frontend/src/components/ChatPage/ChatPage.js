...
function ChatPage(props) {
	...
	const onSubmitChatMessage = (e) => {
		...
		socket.emit("sendMessage", { message: message.value, userId: user.id, roomId: chat.currentRoomId });
	...
	useEffect(()=>{
		...
		fetchChatRoom();
		setSocket(io.connect("/", {path:"/ws/chat", transports: ["websocket"]}));
						...
						<ul onClick={onClickChatRoomList}>
							{
								...
								room.chatRoom.map((e, index)=>
									<li key={index} data-room-id={e.id}>
										<div><FaUserCircle size="42" color="#bdc3c7"/></div>
										<div className="chat-user-inform">
											...
											<div className="chat-product-message">
												{e.chat_messages.length > 0 && e.chat_messages[0].message}
											</div>
										</div>
										...
										<Link to={`/product/${chat.currentChatRoom.article.id}`}>
											<header>
												<div className="chat-image-box">
													<img src={chat.currentChatRoom.article.mainImageUrl} alt="상품이미지" />
												</div>
												<div className="chat-inform">
													<h3>{chat.currentChatRoom.article.title}</h3>
													<div className="user-nickname">{chat.currentChatRoom.article.user.nickname}</div>
												</div>
											</header>
										</Link>
											...
											<ul className="chatList">
												{
													chat.currentRoomMessages.map((e, index)=>
														user.id !== e.user_id?
														<li key={index} className="seller"><FaUserCircle size="34" color="#bdc3c7"/><div className="chat-box">{e.message}</div></li>:		
														<li key={index} className="buyer"><div className="chat-box">{e.message}</div></li>
													)
												}
											</ul>
										</div>
									...
									<div className="chat-form-wrapper">
										<form onSubmit={onSubmitChatMessage} className="chat-form">
											<div className="chat-form__column-left">
												<textarea name="chat-message" placeholder="채팅을 입력하세요."></textarea>
											</div>
