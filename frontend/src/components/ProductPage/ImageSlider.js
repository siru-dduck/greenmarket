...
function ImageSlider(props){
  const { images } = props;
  const [state, dispatch] = useReducer(slider, {index:0, imageNum: images.length});
  
  return (
    <SliderSection>
        <Slider>
          <ImageContainer style={{transform:`translate(-${659 * state.index}px,0px)`}}>
            {
              images.map((image, index) => (
                <ImageWrap key={index}>
                  <img src={image.fileUrl}  alt="제품 이미지" className="product-item__image" />          
                </ImageWrap>   
              ))           
            }
          </ImageContainer>
          <SlickDotList>
            {
              images.map((image, index) => (
                <SlickDot key={index} data-index={index} onClick={(event)=> dispatch({type:"selectIndex", selectedIndex: index })}>
