...
export const getUserById = async (req, res) => {
	...
	try{
		...
		if(!user){
			...
			const {id, email, address1, address2, nickname, profile_image_url} = user.dataValues;
			return res.json({
				isSuccess: true,
...
export const loginUser = async (req, res) => {
	...
	try {
		...
		if (user.dataValues.password === password) {
			...
			res.cookie("x_auth", token,{ expires: new Date(Date.now() + 1000* 60 * 60), httpOnly:true})
				.json({ loginSuccess: true, user:{id, email, address1, address2, nickname, profile_image_url} });
		} else {
			res.json({
				loginSuccess: false,
...
export const logoutUser = (req,res) => {
	...
	if (!token) {
		return res
			.status(400)
			.json({ logoutSuccess: false, code: 400, message: "전송된 토큰이 없습니다." });
	}
	return res.cookie("x_auth", "",{ expires: new Date(Date.now()), httpOnly:true})
...
export const authUser = (req, res) => {
	...
	if (!token) {
		return res
			.status(400)
			.json({ isAuth: false, code: 400, message: "전송된 토큰이 없습니다." });
	}
	try {
